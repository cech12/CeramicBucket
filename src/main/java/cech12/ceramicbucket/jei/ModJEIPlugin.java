...
public class ModJEIPlugin implements IModPlugin {
    ...
    public void registerRecipes(@Nonnull IRecipeRegistration registration) {
        ...
        FilledCeramicBucketItem filledBucketItem = (FilledCeramicBucketItem) CeramicBucketItems.FILLED_CERAMIC_BUCKET;
        ...
        for (Fluid fluid : ForgeRegistries.FLUIDS) {
            ...
            if (bucket instanceof BucketItem && ForgeHooks.getBurnTime(new ItemStack(bucket)) > 0) {
                Fluid bucketFluid = ((BucketItem) bucket).getFluid();
                if (!addedFluids.contains(bucketFluid)) {
                    fuelBuckets.add(filledBucketItem.getFilledInstance(bucketFluid));
